...
public class PixelChange : MonoBehaviour
{
    ...
    public Renderer mRenderer; // renderer in which you will apply changed texture
    Texture2D texture;

    void Awake()
    {
        renderTexture = new RenderTexture(1024, 1024, 0);
        renderTexture.isPowerOfTwo = true;         // This line prevents the error
        renderTexture.Create();                    // This line is totally optional

        texture = new Texture2D(renderTexture.width, renderTexture.height);
        mRenderer.material.SetTexture("_MainTex",texture as Texture);
